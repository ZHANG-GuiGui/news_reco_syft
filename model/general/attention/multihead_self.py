...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask=None):
        ...
        #print(torch.sum(scores, dim=-1, keepdim=True)+1e-8)
        #t = torch.sum(scores, dim=-1, keepdim=True)+1e-8
        attn = torch.div(scores,(torch.sum(scores, dim=-1, keepdim=True)+1e-8))
        context = torch.matmul(attn, V)
        #print("We have succeed in calculating attention scores.")
        #print(attn)
        #print(context)
        return context, attn
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, Q, K=None, V=None, length=None):
        ...
        k_s = self.W_K(Q).view(batch_size, -1, self.num_attention_heads,
                               self.d_k).transpose(1, 2)
        v_s = self.W_V(Q).view(batch_size, -1, self.num_attention_heads,
        ...
        #print(context.get().size())
        #print("We have succeed in calculating attention socre")
        #print(attn)
        #print(context)
